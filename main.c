...
typedef struct consumer   //定义消费者结构体 
{
    int num;
    int rnum;
    char name[100];   //卡的姓名 
    ...
    while(first){
        ...
        switch(first)
        {
            ...
            switch(choice)
            {
                ...
                case 3: input(head);            break;   /*功能三:录入*(学生没有权限)*/ 
                case 4: update(head);          break;   /*功能四:更新*/
                case 5:income(head);           break;   /*功能五:充值*/ 
                case 6: flag = 1;               break;   /*功能六:退出*/ 
            ...
            switch(choice)
            {
                ...
                case 3: MC_input(head);            break;   /*功能三:录入*/
                case 4: MC_update(head);           break;   /*功能四:更新*/
                case 5:income(head);           break;   /*功能五:充值*/
                case 6: printf("程序即将退出");exit(0);break;/*功能六:退出*/
                default: printf("Input Error!\nPlease check again!\n");exit(0);break;
            }
...
void welcome()
{
    ...
    printf("-----------------------------------------------------------------------------\n");
    printf("*****************************************************************************\n");
    printf(" 1---查询   2---排序     3---录入   4---更新    5----充值     6---退出       \n");
    printf("*****************************************************************************\n");
    printf("-----------------------------------------------------------------------------\n");
}
...
void term(pcon x)
{
    ...
    scanf("%s",name);
    printf("按学期查询\n");
    printf("若查询2020年第一学期，请按照月份格式202012输入\n");
    printf("若查询2020年第二学期，请按照月份格式202003输入\n");
    printf("请输入要查询的学期：");
    scanf("%d",&yue);
    beginday = yue * 100;
...
void check_3(pcon x)
{
    ...
    scanf("%s",name);
    printf("按金额区间段查询\n");
    printf("请按照金额格式 8.50 输入；若您只需查询8.5元的  则最大最小填相同即可\n");
    printf("请输入要查询的最小区间：");
    scanf("%f",&beginday);
    printf("请输入要查询的最大区间：");
    scanf("%f",&endday);
...
void MC_term(pcon x)
{
    ...
    printf("按学期查询\n");
    printf("若查询2020年第一学期，请按照日期格式202012输入\n");
    printf("若查询2020年第二学期，请按照日期格式202003输入\n");
    printf("请输入要查询的学期：");
    scanf("%d",&yue);
    beginday = yue * 100;
    if((yue%10) >= 6) {endday = yue + 94;  endday *= 100;}
...
void MC_input_1(pcon x)
{
    ...
    printf("请按照日期格式 20201215 输入交易日期 ");
...
void MC_input_2(pcon x)
{
    ...
    while( ptr != NULL)
    {
        ...
        while( 1 )
        {
            printf("是否输入同一个消费者新的消费记录，是输入1，否输入0\n");
            scanf("%d",&ans);
            if(ans == 0)
            {
                break;
...
void MC_input_4(pcon x)
{
    ...
    while(ptr != NULL)
    {
        ...
        while(pdown != NULL)
        {
            ...
            flag = 1;
            printf("%d %s %d %lld %lld %f %f %s %s\n",pdown->num,pdown->name,pdown->time,pdown->trade_date,pdown->input_date,pdown->fee,pdown->remain,pdown->type,pdown->addr);
            printf("上面为该记录，接下来将修改该记录，请按照日期格式 20200217 输入交易日期：");
            scanf("%lld",&trade_date);
            printf("请输入该消费的金额：");
            scanf("%f",&fee);
            printf("请输入消费后的余额：");
...
void income(pcon x)
{
    ...
    printf("-----------------------------------------------------------------------------\n");
    printf("*****************************************************************************\n");
    printf("请输入你所需要充值的金额:1---50元 2---100元 3---500元 4---其他金额请输入:    \n");
    printf("*****************************************************************************\n");
    printf("-----------------------------------------------------------------------------\n");
    scanf("%d",&inCome);
    switch(inCome)
    {
		...
		case 3:
		case 4:
		default:printf("您输入的金额有误,请重新检查");break;		
   }
}
void income_1(pcon x)
{
	long long trade_date ;
    float fee ;
    float remain;
...
//            while( pdown != NULL )
//            {
//                fprintf(fp,"%d %d %s %d %lld %lld %.2f %.2f %s %s %d\n",pdown->rnum,pdown->num,pdown->name,pdown->time,pdown->trade_date,pdown->input_date,pdown->fee,pdown->remain,pdown->type,pdown->addr,50);
//                pdown = pdown->down;
//            }
//            ptr = ptr->next;
//        }
